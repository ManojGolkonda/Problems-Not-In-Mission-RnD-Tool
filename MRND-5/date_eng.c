...
testcases testcase[] = {
	...
	{ "15.08.1947", "Fifteenth August One Thousand Nine Hundred And Fourty Seven " },
	{ "01/01/2016", "First January Two Thousand Sixteen " }
};
...
void test(int i)
{
	char output[100] = { '\0' };
	char s[11] = { '\0' };
	stringConcat(s, testcase[i].t_date);
	...
	{
		int no = (d1 * 10) + d2 - 1;
		stringConcat(output, date[no].number);
		no = m1 * 10 + m2 - 1;
		stringConcat(output, month[no].number);
		if (y1 != 0)
		{
			no = y1 - 1;
			stringConcat(output, x[no].number);
			stringConcat(output, "Thousand ");
		}
		if (y2 != 0)
		{
			no = y2 - 1;
			stringConcat(output, x[no].number);
			stringConcat(output, "Hundred And ");
		}
		...
		stringConcat(output, "Invalid Date");
	checkResult(output, testcase[i].e_form);
}
